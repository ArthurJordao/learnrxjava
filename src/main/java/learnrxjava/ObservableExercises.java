...
public class ObservableExercises {
    ...
    public Observable<String> exerciseFilterMap(Observable<Integer> nums) {
        return nums.filter(x -> x % 2 == 0).map(x -> x + "-Even");
    }
    ...
    public Observable<JSON> exerciseMovie(Observable<Movies> movies) {
        return movies.flatMap(movie -> movie.videos.flatMap(v -> {
            return v.boxarts.reduce((min, box) -> {
                int boxValue = box.height * box.width;
                int minValue = min.height * min.width;
    ...
    public Observable<String> exerciseZip(Observable<String> a, Observable<String> b) {
        return Observable.zip(a, b, (left, right) -> {
            return left + " " + right;
        });
    }
